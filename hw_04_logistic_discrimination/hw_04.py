#Connor Watson
#HW 04 - Linear Regression Hinge Loss
#10/14/2018
...
def gradient_descent(y, x, w):
    ...
    while abs(prev_obj - obj) > stop: 
        ...
        for i in range(rows):
            if (i in y):
                ...
                for j in range(cols):
                    ...
                    dellf[j] += (y[i] - (1 / (1 + math.exp(-1 * dp)))) * x[i][j]
        ...
        for i in range(rows):
            if (i in y):
                r = dot_product(w, x[i])
                term_0 = -1 * y[i] * math.log(1/(1 + math.exp(-1 * r)))
                term_1 = -1 * (1 - y[i]) * math.log(math.exp(-1 * r)/(1 + math.exp(-1 * r)))
                obj += (term_0 + term_1)
